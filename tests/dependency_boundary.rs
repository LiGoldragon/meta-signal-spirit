...
fn all_features_resolve_one_exact_micro_repository_world() {
    ...
    assert!(
        lock.contains("485be1c609e5f2038fdf54ed0de04cd29d884b06"),
        "lock is missing signal-spirit's exact version-projection producer"
    );

    for forbidden_source in ["nota-next", "[patch.", "branch = ", "path+"] {
        assert!(
