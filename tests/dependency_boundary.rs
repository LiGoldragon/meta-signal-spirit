...
#[test]
fn all_features_resolve_one_exact_micro_repository_world() {
    let output = Command::new("cargo")
    ...
    for exact_direct_pin in [
        ...
        assert!(
            manifest.contains(exact_direct_pin),
            "manifest is missing exact producer pin {exact_direct_pin}"
        );
        assert!(
            lock.contains(exact_direct_pin),
            "lock is missing exact producer pin {exact_direct_pin}"
        );
    }
    ...
    for superseded_pin in [
        ...
        "d2589210c87bcae08386c4b645fc1d7eb49a139c",
...
fn generated_and_runtime_contract_surface_do_not_depend_on_protos() {
    assert!(!meta_signal_spirit::META_SIGNAL_RUST_SOURCE.contains("protos::"));
    assert!(!meta_signal_spirit::META_SIGNAL_RUST_SOURCE.contains("WireContractFamily"));
}
